...
Upon comparing the two datasets we recognize that there are differences in the columns between the two. One important discrepancy is the lack of longitude and latitude positions within the 2018 dataset. We attempted to use the `tidygeocoder` package to get the latitudes and longitudes however attempting that produced N/As owing to the fact that the addresses were not formatted appropriately for the package. 
...
```
We observed from comparing the 2020 data with the 2018 is that the 2018 data has approximately 150,000 more observations. We infer the reasoning behind this is due to the impact that Covid-19 had on this company rather than a lack of interest in ridership, in general.
...
median((dfb %>% mutate(dist = haversine(start_lng, start_lat, end_lng, end_lat)))$dist)
...
We can also create a similar heat map for the total duration/ridership within each census tract. 

```{r warning=FALSE, message=FALSE}
dfj3 = dfj1 %>% replace(., is.na(.), 0)

dfj3 %>% tm_shape() + tm_polygons("tot_dur", alpha=0.6) + 
  tm_basemap(leaflet::providers$OpenStreetMap)
```

There appear to be a few outliers that have more or less total duration than the rest of DC. In particular, there appears to be more usage closer to the middle of DC and less at around the edges. 
